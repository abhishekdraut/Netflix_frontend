...
function CategoryHero({cat_id}) {
  ...
  const baseurl = `https://api.themoviedb.org/3/discover/movie?api_key=d6808d0ad50276bc5f432c356ac53f7a&page=1&with_genres=`;
    ...
    const fetchHeroBackground = async () => {
      ...
      if (response.data) {
        const keywordsresponse = await axios.get(
          `https://api.themoviedb.org/3/movie/${randomElement.id}/keywords?api_key=d6808d0ad50276bc5f432c356ac53f7a`
        );
        console.log(keywordsresponse);
        setKeywords(keywordsresponse.data.keywords.slice(1,6));
      }
    };
    ...
    <>
      ...
      >
        ...
        <div className="HeroTitle">
          <div className="hero_title">{movie.title||movie.original_title }</div>
          ...
          <div className="category_contaier_second movieHerobtns">
            ...
            <Link to={`/moviefulldetail/${movieId}`}><div className="btn playbtn">
              <PlayArrowIcon />
              Play
            </div>
            </Link>
